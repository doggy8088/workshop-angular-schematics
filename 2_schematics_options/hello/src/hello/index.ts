...
export function hello(_options: any): Rule {
  ...
  return (tree: Tree, _context: SchematicContext) => {
    ...
    // TODO 9. retrieve name property form _options ( store it in a name variable )

    // TODO 10. use name variable to parametrize hello.js file content ( replace NG-MY with variable using string template syntax ${} )

    tree.create('hello.js', `console.log('Hello NG-MY!');`);

    // TODO 11. build and run schematics with --name param to create file (mind --dry-run=false because of the dev mode)

    // TODO 12. run created file using node

    // TODO 13. explore running schematics with wrong params like --name1 or without any params at all

    return tree;

    // TODO 14. add new "greeting" option in both schema.d.ts and schema.json, it will be of a string type
    ...
    // TODO 15. add default value (eg Hello) and x-prompt (with the question about greeting we want to use) for the new property in schema.json file

    // TODO 16. get the new "greeting" property from _options and replace hardcoded "Hello" in the generated file content

    // TODO 17. build and run schematics with --name param, but WITHOUT --greeting param (mind --dry-run=false because of the dev mode)
    ...
    // TODO 18. take a look in ./helpers/schema.json_advanced and copy its content to replace properties in schema.json
    ...
    // TODO 19. try to remove "x-prompt" from greetings and run schematics again to see it will use default value when no --greeting option was provided
  };
}
